...
vector<float> getSorted(vector<float> temps, int lines, cl::Context context, cl::CommandQueue queue, cl::Program program, bool minmax)
{
	...
	if (minmax == true) {
		...
		std::cout << "Median value: " << output[(actualSize*0.50) - 1] << endl;
	}
...
int main() {
	...
	try {
		...
		int output = 0;
		int month = 0;
		std::cout << "ALL13460837 Parallel Main Menu" << endl;
		std::cout << "\nFiltering: " << endl;
		...
		std::cout << "2. - Fastest time to calculate everything using Sort and Search method in large dataset (Histogram created by full bitonic sort and search)\n" << endl;
		...
		int finalOption = 0;
		std::cout << "All operations finished, would you like to do something else? 0 for no, 1 for yes\n" << endl;
		std::cin >> finalOption;
